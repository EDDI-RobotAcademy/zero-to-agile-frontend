...
import { getOwnerListingDetail } from '@/lib/repositories/ownerRepository';
...
export default function OwnerListingDetailPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    Promise.resolve(params).then(({ id }) => {
      if (!active) return;
      setListingId(id);
      getOwnerListingDetail(id).then((data) => {
  ...
  if (!listing) {
    return (
      <main className="space-y-4">
        ...
        <Button variant="secondary" onClick={() => router.push('/owner/listings')}>
          목록으로 돌아가기
        </Button>
      </main>
    );
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() => {
              if (!listingId) return;
              router.push(`/owner/listings/${listingId}/matches`);
            }}
          >
            매칭된 임차인 보기
          </Button>
